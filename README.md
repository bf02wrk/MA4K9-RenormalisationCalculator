...
The function evaluate(expression, log, coupling_constant, loop_num) substitutes into the input expression the values $z=0$, $L=\text{log}$, $\lambda=\text{coupling_constant}$ and uses $N=\text{loop_number}$ to replace the zeta values with their numerical values. This gives us a single complex value for each loop number.
