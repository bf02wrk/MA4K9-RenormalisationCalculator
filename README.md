...
The function evaluate(expression, log, couplingConstant, loopNum) substitutes into the input expression the values $z=0$, $L=\text{log}$, $\lambda=\text{couplingConstant}$ and uses $N=\text{loopNum}$ to replace the zeta values with their numerical values. This gives us a single complex value for each loop number.
